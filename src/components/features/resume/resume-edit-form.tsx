...
export interface ResumeEditFormProps {
  initialData: ParsedResume;
  onSave: (data: ParsedResume) => Promise<void>;
  /** Extra buttons to render in the top bar (left side) */
  ...
  const onSubmit = async (data: ResumeFormData) => {
    ...
    try {
      const cleaned: ParsedResume = {
        ...
        awards: data.awards.filter((s) =>
          typeof s === 'string' ? s.trim() !== '' : true
        ),
        certifications: data.certifications.filter((s) =>
          typeof s === 'string' ? s.trim() !== '' : true
        ),
      };
...
function ExperienceCard({
  ...
  const {
    ...
    control,
    name: `experiences.${index}.descriptions` as never,
  });
  ...
  const {
    ...
    control,
    name: `experiences.${index}.skills_involved` as never,
  });
  ...
  const {
    ...
    control,
    name: `experiences.${index}.metrics` as never,
  });
      ...
      <div className="space-y-2">
        ...
        {descFields.map((field, dIdx) => (
          <div key={field.id} className="flex items-center gap-1">
            <Input
              {...register(
                `experiences.${index}.descriptions.${dIdx}` as const
              )}
              placeholder="描述内容"
              className="flex-1"
            />
      ...
      <div className="space-y-2">
        ...
        {metricFields.map((field, mIdx) => (
          <div key={field.id} className="flex items-center gap-1">
            <Input
              {...register(
                `experiences.${index}.metrics.${mIdx}` as const
              )}
              placeholder="量化成果"
              className="flex-1"
            />
